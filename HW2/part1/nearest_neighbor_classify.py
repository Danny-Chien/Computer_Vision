...
def nearest_neighbor_classify(train_image_feats, train_labels, test_image_feats):
    ...
    predict = distance.cdist(test_image_feats, train_image_feats, metric= 'cityblock')
    all_idx = np.argsort(predict, axis = 1)[:, 0:k]
    for idx in all_idx:
        test_predicts.append(mode(label[idx])[0][0])
