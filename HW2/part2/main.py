...
from myModels import  myLeNet, myResnet, mypretrain
from myDatasets import  get_cifar10_train_val_set, get_pseudo_dataset
from myDatasets import get_pseudo_dataset
from tool import train, fixed_seed, get_pseudo_label
...
def train_interface():
    ...
    model = mypretrain(num_out=num_out)
    ...
    # path = './p2_data/unlabeled'
    # unlabeled_set = get_pseudo_dataset(path= path)
    # unlabeled_loader = DataLoader(unlabeled_set, batch_size=batch_size, shuffle= False)
    # filename, labels = get_pseudo_label(unlabeled_loader, model=model, threshold = 0.65)
