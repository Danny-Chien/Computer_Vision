...
## TO DO ##
def plot_learning_curve(x, y, fname):
    """_summary_
...
def train(model, train_loader, val_loader, num_epoch, log_path, save_path, device, criterion, scheduler, optimizer):
    ...
    for i in range(num_epoch):
        ...
        ## TO DO ##
        # validation part 
        with torch.no_grad():   # discard the gradient
            model.eval()
            val_loss = 0
            corr_num = 0
            val_acc = 0 
    ...
    plot_learning_curve(x, overall_acc, fname = 'train_acc')
    plot_learning_curve(x, overall_loss, fname = 'train_loss')
    plot_learning_curve(x, overall_val_acc, fname = 'val_acc')
    plot_learning_curve(x, overall_val_loss, fname = 'val_loss')
    pass


def get_pseudo_label(dataloader, model, threshold):
    model = model.eval()
    filename = []
    labels = []
