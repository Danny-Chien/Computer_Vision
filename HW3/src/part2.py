...
def planarAR(REF_IMAGE_PATH, VIDEO_PATH):
    ...
    while (video.isOpened()):
        ...
        if ret:  ## check whethere the frame is legal, i.e., there still exists a frame
            ...
            warping(ref_image, frame, H, ymin= np.min(corners, axis=0)[1], ymax= np.max(corners, axis=0)[1], xmin= np.min(corners, axis=0)[0], xmax= np.min(corners, axis=0)[0], direction='b')
